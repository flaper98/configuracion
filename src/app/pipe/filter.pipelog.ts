import { Pipe, PipeTransform } from '@angular/core';

@Pipe({
  name: 'filtroFecha'
})
export class FilterPipelog implements PipeTransform {

  transform(log: any, f1: Date, f2: Date) {

    const resultadoFiltro = [];
    let date1 = new Date(f1);
    let date2 = new Date(f2);
    // si es menor a la fecha final
    if (f1 >= f2 || f1 == null) {
      return log;
    }
    // si el argumento de fecha final es invalido, se setea a la fecha actual
    if (f2 == null) {
      f2 = new Date();
    }
    // si ambos arreglos son correctos entonces se crea el nuevo arrego
    for (const logs of log) {
      let a = new Date(logs.fecha);
      console.log(a);

      if (a > date1 && a <= date2) {
        console.log("asd", logs);
        resultadoFiltro.push(logs);
      }
    }
    return resultadoFiltro;
      }



}
